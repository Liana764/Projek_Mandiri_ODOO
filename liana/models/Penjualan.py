...
class Penjualan(models.Model):
    ...
    detailpenjualan_ids = fields.One2many(
        comodel_name='liana.detailpenjualan',
    ...
    def _compute_totalbayar(self):
        for line in self:
            result = sum(self.env['liana.detailpenjualan'].search(
    ...
    def __ondelete_penjualan(self):
        if self.detailpenjualan_ids:
            penjualan = []
            for line in self:
                penjualan = self.env['liana.detailpenjualan'].search(
...
class DetailPenjualan(models.Model):
    _name = 'liana.detailpenjualan'
    _description = 'Detail'

    name = fields.Char(string='Nama')
    penjualan_id = fields.Many2one(
        comodel_name='liana.penjualan',
        string='Detail Penjualan')
    barang_id = fields.Many2one(
        comodel_name='liana.barang',
        string='List Barang')
...
def write(self, vals):
      for line in self:
          data_asli = self.env['liana.detailpenjualan'].search([('penjualan_id', '=', line.id)])
          print(data_asli)
      ...
      for line in self:
          data_setelah_edit = self.env['liana.detailpenjualan'].search([('penjualan_id', '=', line.id)])
          print(data_asli)
